...
boolean searchB(list<string> & B, string word);
...
unmatched(list<string> & A, list<string> & B)
{
	...
	{
		word_A = *it;
		cout << "word_A is " << word_A << endl;

		for (std::list<string>::iterator it = B.begin(); it != B.end(); it++)
		{
			word_B = *it;
			...
			if (word_A > word_B)
			    ...
			    cout << "word_B is " << word_B << endl << endl;
		}
	}*/

	//Perform binary search to find the number of words in A but not in B
	while ((A_index++) < size)
	{
		if (!searchB(B, *itA))
			words_count++;
...
boolean searchB(list<string> & B, string word)
{
	// Binary Search
	list<string>::iterator itB = B.begin();
	int binary_search_index = B.size() / 2;
	std::advance(itB, binary_search_index);  // Moving index to middle of list

	while (binary_search_index > 1)
	{
		binary_search_index = (binary_search_index + 1) / 2;
		if (word < *itB) {
			binary_search_index = abs(binary_search_index) * -1;
		}
		else if (word > *itB) {
			binary_search_index = abs(binary_search_index);
		}
		else {
 			return true;
		}

		std::advance(itB, binary_search_index);

		binary_search_index = abs(binary_search_index);
		if ((binary_search_index == 1) && (word == *itB)) {
			return true;
		}
